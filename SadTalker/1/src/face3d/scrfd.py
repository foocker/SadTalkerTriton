...
class SCRFD:
    def __init__(self, model_file=None, session=None):
        ...
        if self.session is None:
            ...
            self.session = onnxruntime.InferenceSession(model_file, sess_options, providers=[('TensorrtExecutionProvider', \
...
if __name__ == '__main__':
    detector = SCRFD(model_file='./scrfd_500m_bnkps.onnx')
    detector.prepare(1)
    img_paths = ['/data/tritonserver/SadTalker-triton/sadtalker_simplify/examples/source_image/2.jpg']
    for img_path in img_paths:
        img = cv2.imread(img_path)

        for _ in range(3):
            ta = datetime.datetime.now()
            bboxes, kpss = detector.detect_faces(img, 0.5, input_size = (640, 640))
            #bboxes, kpss = detector.detect(img, 0.5)
            tb = datetime.datetime.now()
            print('all cost:', (tb-ta).total_seconds()*1000)
        print(img_path, bboxes.shape)
        x1,y1,x2,y2,score = bboxes[0].astype(np.int32)
        img_crop = img[y1:y2, x1:x2:]
        filename = img_path.split('/')[-1]
        cv2.imwrite('./outputs/%s'%filename, img_crop)
        # if kpss is not None:
        #     print(kpss.shape)
