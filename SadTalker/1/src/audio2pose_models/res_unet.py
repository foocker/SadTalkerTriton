...
class ResUnet(nn.Module):
    ...
    def forward(self, x):
        # print(x.shape, 'x')  # b, 1, seq_len, 6
        # Encode
        x1 = self.input_layer(x) + self.input_skip(x)  # if 84 then ->
        x2 = self.residual_conv_1(x1)  # 42
        x3 = self.residual_conv_2(x2)  # 21
        # Bridge
        x4 = self.bridge(x3) # 11

        # Decode
        x4 = self.upsample_1(x4)  # 22
        # print( x1.shape, x2.shape, x3.shape, x4.shape)
        # 如果输入在维度2上不整除2^3，则这里的cat存在bug,论文默认32 seq_len
